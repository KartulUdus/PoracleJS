		...
		try {
			...
			try {
				if (data.message.sticker && data.message.sticker.length > 0) {
					const { messageId } = await this.bot.telegram.sendSticker(data.target, data.message.sticker, { disable_notification: true })
			...
			try {
				if (data.message.photo && data.message.photo.length > 0) {
					const { messageId } = await this.bot.telegram.sendPhoto(data.target, data.message.photo, { disable_notification: true })
					messageIds.push(messageId)
				}
			} catch (err) {
				this.log.error(`Failed to send Telegram photo ${data.message.photo} to ${data.name}/${data.target}, ${err.message}`)
			}
			const { messageId } = await this.bot.telegram.sendMessage(data.target, data.message.content || data.message || '', {
				parse_mode: 'Markdown',
			...
			if (data.message.location) {
				...
				const { messageId } = await this.bot.telegram.sendLocation(data.target, data.lat, data.lon, { disable_notification: true })
				messageIds.push(messageId)
			}
