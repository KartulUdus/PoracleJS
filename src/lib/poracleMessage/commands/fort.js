	...
	try {
		...
		if (!remove) {
			...
			if ((alreadyPresent.length + updates.length + insert.length) > 50) {
				...
				for (const fort of alreadyPresent) {
					message = message.concat(translator.translate('Unchanged: '), await trackedCommand.fortUpdateRowText(client.config, translator, client.GameData, fort, client.scannerQuery), '\n')
				}
				for (const fort of updates) {
					message = message.concat(translator.translate('Updated: '), await trackedCommand.fortUpdateRowText(client.config, translator, client.GameData, fort, client.scannerQuery), '\n')
				}
				for (const fort of insert) {
					message = message.concat(translator.translate('New: '), await trackedCommand.fortUpdateRowText(client.config, translator, client.GameData, fort, client.scannerQuery), '\n')
				}
			}
