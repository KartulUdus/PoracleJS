...
function createCache(config, ttl) {
	if (config.database.cache === 'redis') {
		return new PoracleRedisCache(ttl, config.database.redis.host,
			config.database.redis.port, config.database.redis.database,
			config.database.redis.user, config.database.redis.password,
