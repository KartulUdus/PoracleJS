...
exports.up = async function migrationUp(knex) {
	...
	await knex.schema.alterTable('monsters', (table) => {
		if (config.database.client !== 'sqlite' && config.database.client !== 'sqlite3') table.increments('uid', { primaryKey: false })
		...
		table.unique([
			'id', 'profile_no', 'pokemon_id', 'min_iv', 'max_iv', 'min_level', 'max_level', 'atk', 'def', 'sta', 'form', 'gender', 'min_weight',
