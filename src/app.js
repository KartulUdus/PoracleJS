...
async function handleAlarms() {
	if (fastify.hookQueue.length && !workingOnHooks && fastify.monsterController && fastify.raidController && fastify.questController) {
		...
		switch (hook.type) {
			case 'pokemon': {
				if (config.alarmtypes.disablePokemon) break
			...
			case 'raid': {
				if (config.alarmtypes.disableRaid) break
			...
			case 'invasion':
			case 'pokestop': {
				if (config.alarmtypes.disablePokestop) break
			...
			case 'quest': {
				if (config.alarmtypes.disableQuest) break
			...
			case 'weather': {
				if (config.alarmtypes.disableWeather) break
