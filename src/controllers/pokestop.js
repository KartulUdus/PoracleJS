...
class Pokestop extends Controller {
	...
	async handle(obj) {
		...
		try {
			...
			if (pregenerateTile && this.geocoding.map.pokestop) {
				data.staticmap = await this.tileserverPregen.getPregeneratedTileURL('pokestop', data, this.geocoding.map.pokestop)
