...
class Monster extends Controller {
	...
	async handle(obj) {
		...
		try {
			...
			if (pregenerateTile && this.config.geocoding.map.pokemon) {
				data.staticmap = await this.tileserverPregen.getPregeneratedTileURL('monster', data, this.config.geocoding.map.pokemon)
