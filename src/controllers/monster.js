		...
		if (['pg', 'mysql'].includes(this.config.database.client)) {
			...
			async handle(obj) {
				...
				try {
					...
					//			if(this.config.general.disableWeather && this.config.weather.enableWeatherForecast && (data.weather || weather)) {
					if((this.config.weather.weatherChangeAlert || this.config.weather.enableWeatherForecast) && data.weather && data.weather !== weatherCellData[currentHourTimestamp]) {
						//this.log.error(`[DEBUG] monsterController maj weather : ${data.weather}`)
						weatherCellData[currentHourTimestamp] = data.weather
						//this.log.error(`[DEBUG] monsterController maj weather ${currentHourTimestamp} de ${weatherCacheData[currentHourTimestamp]} vers ${currentInGameWeather}`)
						//this.log.error('[DEBUG] monsterController maj weather dans cache :', weatherCacheData)
					}
					...
					if (this.config.weather.weatherChangeAlert && weatherCellData.cares && weatherCellData['lastCurrentWeatherCheck'] < currentHourTimestamp && weatherCellData[previousHourTimestamp] > 0 && currentCellWeather > 0 && weatherCellData[previousHourTimestamp] != currentCellWeather) {
						let s2cell = new S2ts.S2Cell(new S2ts.S2CellId(weatherCellId));
						let s2cellCenter = S2ts.S2LatLng.fromPoint(s2cell.getCenter());
						s2cellCenter.latitude = s2cellCenter.latRadians * 180 / Math.PI
						s2cellCenter.longitude = s2cellCenter.lngRadians * 180 / Math.PI
						let polygon = [];
						for (let i = 0; i <= 3; i++) {
							let vertex = S2ts.S2LatLng.fromPoint(s2cell.getVertex(i));
							polygon.push([parseFloat(vertex.latDegrees), parseFloat(vertex.lngDegrees)]);
						}
						const weatherDataPayload = {
							longitude: s2cellCenter.longitude,
							latitude: s2cellCenter.latitude,
					...
					if (this.config.weather.enableWeatherForecast && data.disappear_time > nextHourTimestamp) {
						...
						this.log.error(`[DEBUG] pokemon : ${data.name} (iv ${data.iv})`)
						this.log.error(`[DEBUG] pokemon types : ${data.types}`)
						this.log.error(`[DEBUG] weather forecast : ${weatherForecast.current} to ${weatherForecast.next}`)
						this.log.error(`[DEBUG] pokemonShouldBeBoosted : ${pokemonShouldBeBoosted}`)
						this.log.error(`[DEBUG] hasBoost : ${data.weather}`)
						if (weatherForecast.next > 0 && ((data.weather > 0 && weatherForecast.next !== data.weather) || (weatherForecast.current > 0 && weatherForecast.next !== weatherForecast.current) || (pokemonShouldBeBoosted && data.weather == 0))) {
							...
							this.log.error(`[DEBUG] pokemonWillBeBoosted : ${pokemonWillBeBoosted}`)
							if (data.weather > 0 && !pokemonWillBeBoosted || data.weather == 0 && pokemonWillBeBoosted) {
								this.log.error(`[DEBUG] conditions met for wheater change altering boost`)
								weatherForecast.current = data.weather > 0 ? data.weather : weatherForecast.current
								if (pokemonShouldBeBoosted && data.weather == 0) {
									this.log.error(`[DEBUG] something wrong with current weather info : ${weatherForecast.current} in cache`)
