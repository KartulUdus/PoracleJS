...
class Stats extends EventEmitter {
	...
	calculateRarity() {
		try {
			...
			const rarityGroup4 = Object.keys(totalPokemonCount).filter((pokemonId) => totalPokemonCount[pokemonId].allScanned / currentPokemonTotal > this.config.stats.rarityGroup5UltraRare / 100 && totalPokemonCount[pokemonId].allScanned / currentPokemonTotal <= this.config.stats.rarityGroup4VeryRare / 100).map((x) => parseInt(x, 10)).filter((pokemonId) => !this.config.stats.excludeFromRare.includes(pokemonId))
			const rarityGroup3 = Object.keys(totalPokemonCount).filter((pokemonId) => totalPokemonCount[pokemonId].allScanned / currentPokemonTotal > this.config.stats.rarityGroup4VeryRare / 100 && totalPokemonCount[pokemonId].allScanned / currentPokemonTotal <= this.config.stats.rarityGroup3Rare / 100).map((x) => parseInt(x, 10)).filter((pokemonId) => !this.config.stats.excludeFromRare.includes(pokemonId))
			const rarityGroup2 = Object.keys(totalPokemonCount).filter((pokemonId) => totalPokemonCount[pokemonId].allScanned / currentPokemonTotal > this.config.stats.rarityGroup3Rare / 100 && totalPokemonCount[pokemonId].allScanned / currentPokemonTotal <= this.config.stats.rarityGroup2Uncommon / 100).map((x) => parseInt(x, 10))
