...
class Stats extends EventEmitter {
	...
	calculateRarity() {
		...
		const rarityGroup6 = [...Array(this.config.stats.maxPokemonId).keys()].filter((pokemonId) => pokemonId > 0 && !Object.keys(totalPokemonCount).includes(pokemonId.toString()))
		const rarityGroup5 = Object.keys(totalPokemonCount).filter((pokemonId) => totalPokemonCount[pokemonId] / currentPokemonTotal <= this.config.stats.rarityGroup5UltraRare / 100).map((x) => parseInt(x, 10))
		const rarityGroup4 = Object.keys(totalPokemonCount).filter((pokemonId) => totalPokemonCount[pokemonId] / currentPokemonTotal > this.config.stats.rarityGroup5UltraRare / 100 && totalPokemonCount[pokemonId] / currentPokemonTotal <= this.config.stats.rarityGroup4VeryRare / 100).map((x) => parseInt(x, 10))
		const rarityGroup3 = Object.keys(totalPokemonCount).filter((pokemonId) => totalPokemonCount[pokemonId] / currentPokemonTotal > this.config.stats.rarityGroup4VeryRare / 100 && totalPokemonCount[pokemonId] / currentPokemonTotal <= this.config.stats.rarityGroup3Rare / 100).map((x) => parseInt(x, 10))
