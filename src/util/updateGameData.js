...
const sourceRepo = 'https://raw.githubusercontent.com/PokeMiners/pogo_assets/master/Texts/Latest%20APK/JSON/'
...
function ensure_pokemon(pokemon_id, englishDataPokemonsAndMoves) {
  ...
  const pokemon_name_id_4 = 'pokemon_name_'+pokemonId.toString().padStart(4, '0')
        ...
        try {
          if (object.data.formSettings) {
            ...
            if (forms) {
              ...
              if (!GameMaster.pokemon[pokemon_form_id_default].form) {
                GameMaster.pokemon[pokemon_form_id_default].form = {name: "Normal", proto: '', id: 0};
              }
            }
          }
        } catch (e) {
        ...
        if (object.data.pokemonSettings) {
          ...
          if (form_id) {
            ...
            if (object.data.pokemonSettings.type) {
              ...
              Form.types.push({id: utilData.types[type1name].id, name: type1name});
            }
            if (object.data.pokemonSettings.type2) {
              const type2name = capitalize(object.data.pokemonSettings.type2.replace("POKEMON_TYPE_", ""))
              Form.types.push({id: utilData.types[type2name].id, name: type2name});
            }
          } else {
            ...
            if (object.data.pokemonSettings.type) {
              const type1name = capitalize(object.data.pokemonSettings.type.replace("POKEMON_TYPE_", ""))
              Pokemon.types.push({id: utilData.types[type1name].id, name: type1name});
            }
            if (object.data.pokemonSettings.type2) {
              const type2name = capitalize(object.data.pokemonSettings.type2.replace("POKEMON_TYPE_", ""))
              Pokemon.types.push({id: utilData.types[type2name].id, name: type2name});
            }
          }
          ...
          let move_name_id_4 = 'move_name_'+move_id.toString().padStart(4, '0')
...
(async function () {
  const englishDataPokemonsAndMoves = {}
	...
	Object.keys(englishData).map((index, item) => {
  ...
  englishDataPokemonsAndMoves['pokemon_name_0865'] = 'Sirfetchd'
	...
	for (const lang of Object.keys(supportedLanguages)) {
		...
		if (!currentRawData) {
			...
			const currentDataPokemonsAndMoves = {}
			Object.keys(currentRawData).map((index, item) => {
				if (currentRawData[item].includes('pokemon_name_') || currentRawData[item].includes('move_name_')) {
					currentDataPokemonsAndMoves[currentRawData[item]] = currentRawData[parseInt(index, 10)+1]
				}
			})
