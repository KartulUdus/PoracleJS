...
export const mutations = {
	...
	async createData(state, data) {
		...
		const monstersRaw = _.filter(meta, function(monster){return monster.type === 'pokemon' && !monster.meta.weight})
		for ( let i = 1; i < 800; i += 1){
			...
			let noIvResults = _.filter(monstersRaw, (mon) => { return mon.meta.pokemon_id === i})
