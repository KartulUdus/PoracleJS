	...
	async findActiveComEvent() {
		return new Promise(resolve => {
			this.db.query('SELECT * FROM comevent WHERE end_timestamp > now() and finished = false')
				...
				.catch((err) => {log.error(`findActiveComEvent errored with: ${err}`)})
		})
	}
	...
	async findExpiredComEvent() {
		return new Promise(resolve => {
			this.db.query('SELECT * FROM comevent WHERE end_timestamp < now() and finished = false')
				...
				.catch((err) => {log.error(`findExpiredComEvent errored with: ${err}`)})
		})
	}


	async getComEventResults() {
		return new Promise(resolve => {
			const query = `
			...
			this.db.query(query)
				...
				.catch((err) => {log.error(`getComEventResults errored with: ${err}`)})
		})
	}
