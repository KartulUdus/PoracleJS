	...
	async handle(data) {
		return new Promise((resolve) => {
			...
			this.insertOrUpdateQuery(
				...
				[`'${data.encounter_id}'`, `'${data.pokemon_id}'`, `${data.individual_attack}`, `'${data.individual_defense}'`, `'${data.individual_stamina}'`, `${data.pokemon_level}`, `'${data.gender}'`, `'${data.form}'`, `${data.weight}`, `'${data.weather_boosted_condition}'`, `'${data.cp}'`, `${data.move_1}`, `'${data.move_2}'`, `FROM_UNIXTIME(${data.disappear_time})`, 'UTC_TIMESTAMP()', `${data.latitude}`, `${data.longitude}`]
			).catch((err) => {
				log.error(`Updating Pokemon table errored with: ${err}`)
			})
