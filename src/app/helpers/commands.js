...
*/
	...
	if (msg.content === `${config.discord.prefix}poracle` && msg.channel.name === config.discord.channel) {
		query.countQuery('id', 'humans', 'id', msg.author.id)
			...
			.catch((err) => { log.error(`Commander !poracle errored with: ${err}`)})
	}
		...
		query.countQuery('id', 'humans', 'id', msg.author.id).then((isregistered) => {
			...
			let rawArgs = msg.content.slice(`${config.discord.prefix}track`.length).split(' ')
			let args = rawArgs.join('|').toLowerCase().split('|')
			let monsters = []
			let distance = 0
			let cp = 0
			let maxcp = 9000
			...
			args.forEach((element) => {
				let pid = _.findKey(monsterData, mon => mon.name.toLowerCase() === element)
...
*/
		...
		query.countQuery('id', 'humans', 'id', msg.channel.id).then((isregistered) => {
			...
			if (monsters.length) {
				monsters.forEach((monster) => {	query.deleteMonsterQuery('raid', 'pokemon_id', `${monster}`, msg.channel.id)});
				msg.react('✅');
			}
			if (levels.length) {
				levels.forEach((level) => {	query.deleteMonsterQuery('raid', 'level', `${level}`, msg.channel.id)});
				msg.react('✅');
			}
			if (!monsters.length && !levels.length) msg.reply('404 No raid bosses or levels found');
		})
...
query.countQuery('TABLE_NAME', 'information_schema.tables', 'table_schema', config.db.database).then ( (tables) => {
	if (!tables) {
		Promise.all([
			query.mysteryQuery(migrations.humans)
		])
	}
})
