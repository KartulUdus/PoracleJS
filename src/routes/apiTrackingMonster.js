	...
	fastify.post('/api/tracking/pokemon/:id', options, async (req) => {
		...
		const insert = insertReq.map((row) => {
			...
			return {
				...
				pvp_ranking_best: +defaultTo(row.pvp_ranking_league, 1),
				pvp_ranking_worst: +defaultTo(row.pvp_ranking_league, 4096),
				pvp_ranking_min_cp: +defaultTo(row.pvp_ranking_league, 0),
